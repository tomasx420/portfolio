	...
	<div class="grid md:grid-cols-2 gap-24">
		<div class="relative" style="height: 500px; perspective: 1000px;">
			<div
				...
				class:flipped={isStrengthsFlipped}
				style="transform-style: preserve-3d; transition: transform 0.6s;"
			>
				<div
					class="absolute inset-0 w-full h-full backface-hidden bg-white rounded-lg shadow-md"
				>
					<div class="h-full flex flex-col">
						<div class="px-6 py-4 bg-gray-200">
							<h3
								class="flex items-center text-secondary text-2xl font-semibold glow-green"
							>
							...
							<ul class="space-y-12">
								{#each strengths as strength}
									<li class="flex items-start">
										<CheckCircle
											class="mr-2 h-5 w-5 text-secondary flex-shrink-0 mt-0.5"
											size={16}
										/>
										<span>{strength}</span>
									</li>
				...
				>
					<div class="h-full bg-background p-6 flex flex-col">
						<h3 class="text-secondary text-2xl font-semibold mb-4">
							How I Use My Strenghts
						</h3>
						<p class="text-text flex-grow">
							I make the most of my strengths by taking the lead in team settings,
							solving problems with efficiency, and always being there to support the
							people im working with. Planning and presenting with confidence helps me
							stay on top of deadlines and communicate my ideas effectively. I'm always
							working on improving these skills and pushing myself to take on bigger
			...
			>
				<button
					on:click={() => flipCard("strengths")}
					class="bg-secondary text-white px-4 py-2 rounded hover:bg-primary transition-colors"
				>
		...
		<div class="relative" style="height: 500px; perspective: 1000px;">
			<div
				class="absolute inset-0 w-full h-full"
				class:flipped={isWeaknessesFlipped}
				style="transform-style: preserve-3d; transition: transform 0.6s;"
			>
				<div
					class="absolute inset-0 w-full h-full backface-hidden bg-white rounded-lg shadow-md"
						...
						<div class="px-6 py-4 bg-background flex-grow pt-8">
							<ul class="space-y-12">
								{#each weaknesses as weakness}
									<li class="flex items-start">
										<XCircle
											class="mr-2 h-5 w-5 text-text flex-shrink-0 mt-0.5"
											size={16}
										/>
										<span>{weakness}</span>
									</li>
				...
				>
					<div class="h-full bg-background p-6 flex flex-col">
						...
						<p class="text-text flex-grow">
							I think that recognizing these weaknesses is the first step toward
							improvement. I'm working on staying present in difficult conversations,
							rather than stepping away. I’m also finding a balance between
							perfectionism and efficiency, and improving my delegation by learning to
							trust others while providing clear guidance
						</p>
					</div>
				</div>
			</div>
